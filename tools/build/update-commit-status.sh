...
function show_help() {
    ...
    echo -e "\n\tThis script requires the following environment variables: SERVICE_NAME, GITHUB_TOKEN, AWS_REGION, CODEBUILD_BUILD_ID\n"
...
function create_commit_status() {
    result=`hub api "https://api.github.com/repos/$owner/$SERVICE_NAME/statuses/$commit?access_token=$GITHUB_TOKEN" \
